...
namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public class AndroidIntent : IDisposable
	{
		...
		public AndroidIntent SetPreferredActivity(string packageName, string containsPhrase)
		{
			const int matchDefaultOnlyFlag = 0x00010000;
			
			using (var pm = AGUtils.PackageManager)
			{
				var resolveInfoList = pm.CallAJO("queryIntentActivities", AJO, matchDefaultOnlyFlag);
				
				var ajos = resolveInfoList.FromJavaList<AndroidJavaObject>();
				foreach (var ajo in ajos)
				{
					var activityInfo = ajo.Get<AndroidJavaObject>("activityInfo");
					var fullPackageName = activityInfo.Get<string>("packageName");
					var activityName = activityInfo.Get<string>("name");
					
					if (fullPackageName.StartsWith(packageName) && activityName.Contains(containsPhrase))
					{
						SetClassName(fullPackageName, activityName);
					}
				}
			}
